...
class QLearningAgent(object):
    ...
    def update(self, state_dict, action, iteration):
        """
        Updating Q-values according to Belman equation
        Required parameters:
            - state_dict as dict
        ...
        if iteration % 1000 == 0:
            print("called")
            self.q.to_csv('q-table itr '+str(iteration)+'.csv')
# 3. Deck
# ----------------------------------------------------------------------------
...
class Player(object):
    ...
    def draw_card_agent(self, deck, player, iteration):
        global card_pool
        
        card_pool = deck.draw()
        
        player.play_agent(player, deck, iteration)
    ...
    def take_card_agent(self, player, deck, iteration):
        global card_pool
        global chip_pool
        ...
        if algorithm == "q-learning":
            agent.update(self.state, self.action, iteration)
        
        if deck.check_end() != True:
            player.draw_card_agent(deck, player, iteration)
    ...
    def play_agent(self, player, deck, iteration):
        ...
        if self.action == "take":
            player.take_card_agent(player, deck, iteration)
...
class Game(object):
   ...
   def __init__(self, player_1, player_2, player_3, rand_start, iteration):
        """
        ...
        if rand_start == "random":
            turn_no = random.randint(1, 3)
            if turn_no == 1:
                self.player_1.draw_card_rand(deck, self.player_1)
            elif turn_no == 2:
                self.player_2.draw_card_rand(deck, self.player_2)
            elif turn_no == 3:
                self.player_3.draw_card_agent(deck, self.player_3, iteration)
        ...
        while deck.check_end() != True:
            ...
            if turn_no % 3 == 1:
                self.player_1.rand_play(self.player_1, deck)
                
            if turn_no % 3 == 2:
                self.player_2.rand_play(self.player_2, deck)
                
            if turn_no % 3 == 0:
                self.player_3.play_agent(self.player_3, deck, iteration)
...
def Tournament(player_1, player_2, player_3, match_no, algo, agent_info, rand_start):
    ...
    for i in range(1, match_no+1):
        iteration = int(i)
        Game(player_1, player_2, player_3, rand_start, iteration)
        if i % 100 == 0:
            scores = tally[:]
            scores.append(iteration)
            winratelists.append(scores)
    print(winratelists)
    with open("randomn3Charlie.csv", "w") as f:
        writer = csv.writer(f)
        header = ['Alice', 'Bob', 'Charlie', 'Iteration']
        writer.writerow(header)
        writer.writerows(winratelists)

             
agent_init_info = {"epsilon":0.2, "step_size":0.2, "new_model":True}
            
Tournament('Alice', 'Bob', 'Charlie', 10000, "monte-carlo", agent_init_info, "random")