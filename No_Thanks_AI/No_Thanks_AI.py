...
class Player(object):
    ...
    def draw_card_weighted(self, deck, player):
        global card_pool
    ...
    def take_card_weighted(self, player, deck):
        global card_pool
        global chip_pool
        global game_end
        ...
        if not deck.check_end():
            player.draw_card_weighted(deck, player)
        else:
            game_end = True
    ...
    def weighted_play(self, player, deck):
        ...
        # print(card_point_tally(self.card_hand))        
        
        take_value = card_point_tally(self.card_hand + [card_pool]) - (Player.chip_weight(take_chip_hand)/2) * take_chip_hand
        pass_value = card_point_tally(self.card_hand) - Player.chip_weight(pass_chip_hand) * pass_chip_hand
        
        
        if take_value <= pass_value or self.chip_hand == 0:
            player.take_card_weighted(player, deck)
